...
def find_scalars(independent_samples, dependent_samples):
	...
	for target in independent_samples:
		...
		times = [float(row[2]) for row in psmc_output]
...
def curvefitting(independent_samples, dependent_samples, scalars):
	for sample_pair in sample_pairs:
		...
		for scalar in scalar_set:
			...
			topNscalars.append([sample_pair, float("%.3f" % (scalar)), int(np.mean(slope_differentials)*100)])
		topNscalars.sort(key=lambda x: x[2])
		for item in topNscalars[:top_N]: print "%.3f --- %i" % (item[1], item[2])
